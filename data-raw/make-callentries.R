...
defs <- tibble(
  def = src_sources %>%
    str_extract_all(regex("SEXP teos10_c_[^\\)]+\\)\\s+", multiline = TRUE)) %>%
    unlist() %>%
    str_replace_all("\\s+", " ") %>%
    str_trim(),
  name = def %>% str_extract("teos10_c_[^\\(]+"),
  return_type = "SEXP",
  args = def %>%
    str_remove("SEXP teos10_c_[^\\(]+\\(") %>%
    str_remove("\\)$") %>%
    str_split("\\s*,\\s*") %>%
...
init <- read_file("src/teos10-init.c")
pattern <- regex(
